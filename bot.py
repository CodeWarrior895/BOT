...
RULES = """
👋 **Привет! Добро пожаловать в бота для отправки валентинок!** 💌  

📜 *Правила использования:*  
1️⃣ **Не выдавай себя за другого человека.**  
2️⃣ **Пиши только приятные слова** – никаких оскорблений или негатива!  
3️⃣ **Используй бота только по назначению** – для отправки валентинок.  
4️⃣ **Указывай свои настоящие данные (имя, фамилию и класс),** чтобы отправитель мог тебя найти.  

💡 **Важно!**  
Не бойся указывать свои настоящие данные – **они не будут показаны человеку, которому ты пишешь валентинку**. Эти данные нужны **только для поиска получателей**, чтобы твоё сообщение точно дошло до нужного человека! ❤️  
"""
...
def start(message):
    ...
    user_data[user_id] = {}
    bot.send_message(user_id, f"{RULES}\nКак тебя зовут?")
...
def get_name(message):
    ...
    if " " in name:
        bot.send_message(
            user_id,
            "❌ Ошибка: Имя не должно содержать пробелов. Введите корректное имя:"
        )
        bot.register_next_step_handler(message, get_name)
        return
...
def get_surname(message):
    ...
    if " " in surname:
        bot.send_message(
            user_id,
            "❌ Ошибка: Фамилия не должна содержать пробелов. Введите корректную фамилию:"
        )
        bot.register_next_step_handler(message, get_surname)
        return
...
def get_class(message):
    ...
    if not class_num.isdigit():
        bot.send_message(
            user_id, "❌ Ошибка: Введите только номер класса (например: 10).")
        bot.register_next_step_handler(message, get_class)
        return

    user_data[user_id]['class'] = class_num
    bot.send_message(user_id,
                     "Какая буква у твоего класса? (одна буква, например: А)")
...
def save_complaint(message):
    ...
    complaints_ref.push({"user_id": user_id, "complaint": complaint_text})
    bot.send_message(user_id, "✅ Ваша жалоба отправлена.")
    send_search_button(user_id)


@bot.message_handler(
    func=lambda message: message.text == "🔍 Найти пользователя")
...
def search_users_by_class(message):
    ...
    if users_ref:
        ...
        for uid, user_info in users_ref.items():
            if user_info.get("class") == class_num and user_info.get(
                    "class_letter").upper() == class_letter:
        ...
        if matching_users:
            ...
            for uid, user_info in matching_users:
                button = KeyboardButton(
                    f"{user_info['name']} {user_info['surname']} ({uid})")
                markup.add(button)
            bot.send_message(user_id,
            ...
            bot.send_message(user_id,
                             "❌ Пользователи этого класса не найдены.")
            send_search_button(user_id)
    else:
...
def ask_for_valentine(message):
    ...
    user_id = message.chat.id
    selected_uid = selected_text.split(" (")[1].rstrip(
        ")") if " (" in selected_text else None
...
def send_valentine(message):
    ...
    if recipient_id:
        bot.send_message(int(recipient_id),
